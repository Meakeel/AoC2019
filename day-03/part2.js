...
function getCloest(mapOne, mapTwo) {
  let mappedMapOne = mapOne.slice().map((position) => JSON.stringify(position.position));
  let mappedMapTwo = mapTwo.slice().map((position) => JSON.stringify(position.position));

  let intersections = mappedMapOne
    .filter((position) => mappedMapTwo.includes(position))
    .map((position) => position.replace('[', '').replace(']', '').split(',').map(Number));

  let results = [];

  intersections.forEach(row => {
    let mapOneSteps = mapOne.filter(pos => pos.position[0] == row[0] && pos.position[1] == row[1])[0].steps;
    let mapTwoSteps = mapTwo.filter(pos => pos.position[0] == row[0] && pos.position[1] == row[1])[0].steps;

    results.push(mapOneSteps + mapTwoSteps);
  });
